...
At its heart, KAT is an example of how an application can be developed and deployed into Kubernetes. The main focus is on how an application is developed and hosted in Kubernetes rather than everything Kubernetes can do. This repository includes application code, Helm charts that deploy the application into Kubernetes, monitoring for observability and a database, complete with persistent volumes deployed in Kubernetes.

Developing applications for Kubernetes needs some additional work because along with application code, infrastructure-as-code is also part of the deliverable. This includes configuring Kubernetes-aware load-balancers and application routing, managing secrets and environment variables, building rules for auto-scaling and resource management and software-defined-storage.
...
4. The application can be deployed on any Kubernetes cluster. KAT includes scripts for creating a Microk8s Kubernetes cluster on a single node (for example, on a virtual machine).
...
| Installation scripts | [code/local-kubernetes-cluster-installation](../code/local-kubernetes-cluster-installation) | Bash scripts to install the Microk8s cluster and application code |
...
We have collected and distilled concise explanations of concepts that we believe are useful to know before proceeding on your Kubernetes journey. Many of these concepts are large multi-year open-source projects in their own right with hundreds of contributors and hundreds of thousands of lines of code. So our treatment of these topics is fleeting at best,  we do provide you with links to official documentation and a few other curated resources to learn more in depth if you wish to.

## Containers
![Container build and run](images/Slide5.PNG)**Fig. 1: Building container images and running containers**

Containers are like boxes. You take your application's code and all its dependencies and pack them up into a container image. This container image is then "shipped" from a developer's laptop or a build system into QA, staging and production without change (container images are immutable, although you can inject environment variables and disk mount points into a running container). When it’s time to "run" the application, these container images become running containers - a process running on the host computer that is isolated from other containers as well as the host computer itself. Container processes can also be rate-limited in how much CPU or memory they use so that a single container cannot starve the entire host of CPU or memory. Isolation and rate-limiting is achieved by invoking the container process in namespaces and through [cgroups](https://www.linuxjournal.com/content/everything-you-need-know-about-linux-containers-part-i-linux-control-groups-and-process).
